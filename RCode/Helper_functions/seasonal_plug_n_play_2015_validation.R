...
#Seasonal_AR_Minwind_MinSchmidt_Diff
  data.Seasonal_AR_Minwind_MinSchmidt_Diff <- list(y=y, year_no = year_no,week_min_W = week_min_W, week_min_S = week_min_S, beta.m1=0,  beta.m2=0,beta.m3=0,beta.m4=0, beta.v1=0.001, beta.v2=0.001,beta.v3=0.001,beta.v4=0.001, Wnd = Wnd, Schmidt=Schmidt,season_weeks=season_weeks,x_ic=-5,tau_ic = 100,a_proc = 0.001,r_proc = 0.001, a_obs = 15.37, r_obs = 7.84)
  variable.names.Seasonal_AR_Minwind_MinSchmidt_Diff <- c("tau_proc", "beta1","beta2", "beta3","beta4", "tau_obs","tau_W_proc", "tau_S_proc")
  variable.namesout.Seasonal_AR_Minwind_MinSchmidt_Diff <- c("tau_proc", "beta1", "beta2","beta3","beta4",  "mu", "tau_obs", "tau_W_proc", "tau_S_proc")
  init.Seasonal_AR_Minwind_MinSchmidt_Diff <- list(list(tau_proc=0.001, tau_obs = 0.1,  tau_W_proc = 0.01,tau_S_proc = 0.01, beta1=-0.5, beta2=-0.5, beta3=-0.5, beta4=-0.5), list(tau_proc=0.1,  tau_obs = 1,tau_W_proc = 0.1,tau_S_proc = 0.1, beta1=0, beta2=0, beta3=0, beta4=0), list(tau_proc=1, tau_obs = 5,tau_W_proc = 1,tau_S_proc = 1, beta1=0.5,beta2=0.5, beta3=0.5, beta4=0.5))
  params.Seasonal_AR_Minwind_MinSchmidt_Diff <- c("tau_proc","beta1", "beta2", "beta3","beta4","tau_obs","tau_W_proc", "tau_S_proc")
  
#Seasonal_AR_SWradiation_MinSchmidt_Diff
  data.Seasonal_AR_SWradiation_MinSchmidt_Diff <- list(y=y, year_no = year_no,week_avg = week_avg, week_min_S = week_min_S, beta.m1=0,  beta.m2=0,beta.m3=0,beta.m4=0,beta.m5=0, beta.v1=0.001, beta.v2=0.001,beta.v3=0.001,beta.v4=0.001,beta.v5=0.001, SW=SW, Schmidt=Schmidt,season_weeks=season_weeks,x_ic=-5,tau_ic = 100,a_proc = 0.001,r_proc = 0.001, a_obs = 15.37, r_obs = 7.84)
  variable.names.Seasonal_AR_SWradiation_MinSchmidt_Diff <- c("tau_proc", "beta1","beta2", "beta3","beta4","beta5", "tau_obs","tau_SW_proc", "tau_S_proc")
  variable.namesout.Seasonal_AR_SWradiation_MinSchmidt_Diff <- c("tau_proc", "beta1", "beta2","beta3","beta4","beta5",  "mu", "tau_obs", "tau_SW_proc", "tau_S_proc")
  init.Seasonal_AR_SWradiation_MinSchmidt_Diff <- list(list(tau_proc=0.001, tau_obs = 0.1,  tau_SW_proc = 0.01,tau_S_proc = 0.01, beta1=-0.5, beta2=-0.5, beta3=-0.5, beta4=-0.5, beta5=-0.5), list(tau_proc=0.1,  tau_obs = 1,tau_SW_proc = 0.1,tau_S_proc = 0.1, beta1=0, beta2=0, beta3=0, beta4=0, beta5=0), list(tau_proc=1, tau_obs = 5,tau_SW_proc = 1,tau_S_proc = 1, beta1=0.5,beta2=0.5, beta3=0.5, beta4=0.5, beta5=0.5))
  params.Seasonal_AR_SWradiation_MinSchmidt_Diff <- c("tau_proc","beta1", "beta2", "beta3","beta4","beta5","tau_obs","tau_SW_proc", "tau_S_proc")
...
if(model_name=="Seasonal_RandomWalk"){
  ...
  pred.Seasonal_RandomWalk <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_RandomWalk <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- c(1)
  season_weeks <- c(1:20)
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      if(is.na(mydata[j-1])){
      pred.Seasonal_RandomWalk[,j] = rnorm(nsamp,pred.Seasonal_RandomWalk[,j-1],sqrt(1/tau_proc))
      }else{
      pred.Seasonal_RandomWalk[,j] = rnorm(nsamp,mydata[j-1],sqrt(1/tau_proc))
      }
      
      #data model
      pred_obs.Seasonal_RandomWalk[,j] = rnorm(nsamp,pred.Seasonal_RandomWalk[,j],sqrt(1/tau_obs))
...
if(model_name=="Seasonal_RandomWalk_Obs_error"){
  tau_proc = out[samp,grep("tau_proc",colnames(out))]
  tau_obs = out[samp,grep("tau_obs",colnames(out))]
  pred.Seasonal_RandomWalk_Obs_error <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_RandomWalk_Obs_error <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- c(1)
  season_weeks <- c(1:20)
  mydata <- ys
  ...
  for(j in 2:max(season_weeks)){
    ...
    #process model
    if(is.na(mydata[j-1])){
      pred.Seasonal_RandomWalk_Obs_error[,j] = rnorm(nsamp,pred.Seasonal_RandomWalk_Obs_error[,j-1],sqrt(1/tau_proc))
    }else{
      pred.Seasonal_RandomWalk_Obs_error[,j] = rnorm(nsamp,mydata[j-1],sqrt(1/tau_proc))
    }
    
    #data model
    pred_obs.Seasonal_RandomWalk_Obs_error[,j] = rnorm(nsamp,pred.Seasonal_RandomWalk_Obs_error[,j],sqrt(1/tau_obs))
...
if(model_name=="Seasonal_AR"){
  ...
  pred.Seasonal_AR <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      if(is.na(mydata[j-1])){
        lambda[,j] <- beta1 + beta2*pred.Seasonal_AR[,j-1] 
      } else{
      lambda[,j] <- beta1 + beta2*mydata[j-1] 
      }
      pred.Seasonal_AR[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_AR[,j] = rnorm(nsamp,pred.Seasonal_AR[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_AR_Mintemp"){
  ...
  pred.Seasonal_AR_Mintemp <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_Mintemp <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  Tempz = Temps
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(Tempz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min[j],sqrt(1/tau_T_proc))}
      else if(!is.na(Tempz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp[,j-1]+ beta3*Tempz[j] }
      else if(is.na(Tempz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_Lag[,j-1]+ beta3*rnorm(nsamp,week_min[j],sqrt(1/tau_T_proc))}
      else{lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Tempz[j] }
      
      pred.Seasonal_AR_Mintemp[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_AR_Mintemp[,j] = rnorm(nsamp,pred.Seasonal_AR_Mintemp[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_AR_Mintemp_Lag"){
  ...
  beta2 = out[samp,grep("beta2",colnames(out))]
  beta3 = out[samp,grep("beta3",colnames(out))]
  pred.Seasonal_AR_Mintemp_Lag <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_Mintemp_Lag <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  Tempz = Temps
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(Tempz[j-1]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min[j-1],sqrt(1/tau_T_proc))}
      else if(!is.na(Tempz[j-1]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_Lag[,j-1]+ beta3*Tempz[j-1] }
      else if(is.na(Tempz[j-1]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_Lag[,j-1]+ beta3*rnorm(nsamp,week_min[j-1],sqrt(1/tau_T_proc))}
      else {lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Tempz[j-1] }
      
      pred.Seasonal_AR_Mintemp_Lag[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))

      #data model
      pred_obs.Seasonal_AR_Mintemp_Lag[,j] = rnorm(nsamp,pred.Seasonal_AR_Mintemp_Lag[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_AR_Minwind"){
  ...
  beta2 = out[samp,grep("beta2",colnames(out))]
  beta3 = out[samp,grep("beta3",colnames(out))]
  pred.Seasonal_AR_Minwind <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_Minwind <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  Wndz = Wnds
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(Wndz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min[j],sqrt(1/tau_W_proc))}
      else if(!is.na(Wndz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind[,j-1]+ beta3*Wndz[j] }
      else if(is.na(Wndz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind[,j-1]+ beta3*rnorm(nsamp,week_min[j],sqrt(1/tau_W_proc))}
      else {lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Wndz[j] }
      
      pred.Seasonal_AR_Minwind[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_AR_Minwind[,j] = rnorm(nsamp,pred.Seasonal_AR_Minwind[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_GDD_Quad"){
  ...
  pred.Seasonal_GDD_Quad <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_GDD_Quad <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  GDDz <- GDDs
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(GDDz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_G_proc)) + beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_G_proc))^2}
      else if(!is.na(GDDz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_GDD_Quad[,j-1]+ beta3*GDDz[j]+ beta4*GDDz[j]^2 }
      else if(is.na(GDDz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_GDD_Quad[,j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_G_proc))+ beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_G_proc))^2}
      else {lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*GDDz[j]+ beta3*GDDz[j]^2 }
      
      pred.Seasonal_GDD_Quad[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_GDD_Quad[,j] = rnorm(nsamp,pred.Seasonal_GDD_Quad[,j],sqrt(1/tau_obs))
    }
  }
...
if(model_name=="Seasonal_SWradiation_Quad"){
  ...
  beta3 = out[samp,grep("beta3",colnames(out))]
  beta4 = out[samp,grep("beta4",colnames(out))]
  pred.Seasonal_SWradiation_Quad <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_SWradiation_Quad <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  SWz = SWs
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(SWz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc)) + beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2}
      else if(!is.na(SWz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_SWradiation_Quad[,j-1]+ beta3*SWz[j]+ beta4*SWz[j]^2 }
      else if(is.na(SWz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_SWradiation_Quad[,j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+ beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2}
      else {lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*SWz[j]+ beta3*SWz[j]^2 }
      
      pred.Seasonal_SWradiation_Quad[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_SWradiation_Quad[,j] = rnorm(nsamp,pred.Seasonal_SWradiation_Quad[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_AR_MaxSchmidt_Lag"){
  ...
  beta2 = out[samp,grep("beta2",colnames(out))]
  beta3 = out[samp,grep("beta3",colnames(out))]
  pred.Seasonal_AR_MaxSchmidt_Lag <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_MaxSchmidt_Lag <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=ncol(mu))
  Schmidtz = Schmidts
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(Schmidtz[j-1]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_max[j-1],sqrt(1/tau_S_proc))}
      else if(!is.na(Schmidtz[j-1]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_MaxSchmidt_Lag[,j-1]+ beta3*Schmidtz[j-1] }
      else if(is.na(Schmidtz[j-1]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_MaxSchmidt_Lag[,j-1]+ beta3*rnorm(nsamp,week_max[j-1],sqrt(1/tau_S_proc))}
      else {lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Schmidtz[j-1] }
      
      pred.Seasonal_AR_MaxSchmidt_Lag[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_AR_MaxSchmidt_Lag[,j] = rnorm(nsamp,pred.Seasonal_AR_MaxSchmidt_Lag[,j],sqrt(1/tau_obs))
    }
  }
...
if(model_name=="Seasonal_AR_MinSchmidt_Diff"){
  ...
  beta2 = out[samp,grep("beta2",colnames(out))]
  beta3 = out[samp,grep("beta3",colnames(out))]
  pred.Seasonal_AR_MinSchmidt_Diff <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_MinSchmidt_Diff <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=ncol(mu))
  Schmidtz = Schmidts
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(Schmidtz[j-1]) & is.na(Schmidtz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_MinSchmidt_Diff[,j-1]+ beta3*(rnorm(nsamp,week_min[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min[j-1],sqrt(1/tau_S_proc)))}
      else if(is.na(Schmidtz[j-1]) & is.na(Schmidtz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*(rnorm(nsamp,week_min[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min[j-1],sqrt(1/tau_S_proc)))}
      else if(!is.na(Schmidtz[j-1]) & is.na(Schmidtz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*(rnorm(nsamp,week_min[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
      else if(is.na(Schmidtz[j-1]) & !is.na(Schmidtz[j]) & !is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*(Schmidtz[j] - rnorm(nsamp,week_min[j-1],sqrt(1/tau_S_proc)))}
      else if(is.na(Schmidtz[j-1]) & !is.na(Schmidtz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_MinSchmidt_Diff[,j-1]+ beta3*(Schmidtz[j] - rnorm(nsamp,week_min[j-1],sqrt(1/tau_S_proc)))}
      else if(!is.na(Schmidtz[j-1]) & is.na(Schmidtz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_MinSchmidt_Diff[,j-1]+ beta3*(rnorm(nsamp,week_min[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
      else if(!is.na(Schmidtz[j-1]) & !is.na(Schmidtz[j]) & is.na(mydata[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_MinSchmidt_Diff[,j-1]+ beta3*(Schmidtz[j] - Schmidtz[j-1])}
      else {lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*(Schmidtz[j] - Schmidtz[j-1]) }
      
      pred.Seasonal_AR_MinSchmidt_Diff[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_AR_MinSchmidt_Diff[,j] = rnorm(nsamp,pred.Seasonal_AR_MinSchmidt_Diff[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_AR_Mintemp_MinSchmidt_Diff"){
  ...
  beta3 = out[samp,grep("beta3",colnames(out))]
  beta4 = out[samp,grep("beta4",colnames(out))]
  pred.Seasonal_AR_Mintemp_MinSchmidt_Diff <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_Mintemp_MinSchmidt_Diff <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  Tempz = Temps
  ...
  mydata <- ys
    ...
    for(j in 2:max(season_weeks)){
      ...
      #process model
      #filling Temp NAs
      if(is.na(mydata[j-1]) & is.na(Tempz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc)) + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
      else if(!is.na(mydata[j-1]) & is.na(Tempz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc)) + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
      else if(!is.na(mydata[j-1]) & !is.na(Tempz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Tempz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
      else if(!is.na(mydata[j-1]) & !is.na(Tempz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Tempz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
      else if(!is.na(mydata[j-1]) & !is.na(Tempz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Tempz[j] + beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
      else if(!is.na(mydata[j-1]) & is.na(Tempz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc))+ beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc))) }
      else if(!is.na(mydata[j-1]) & is.na(Tempz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc))+ beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1]) }
      else if(!is.na(mydata[j-1]) & is.na(Tempz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc))+ beta4*(Schmidtz[j] - Schmidtz[j-1])}
      else if(is.na(mydata[j-1]) & !is.na(Tempz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*Tempz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
      else if(is.na(mydata[j-1]) & !is.na(Tempz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*Tempz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
      else if(is.na(mydata[j-1]) & !is.na(Tempz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*Tempz[j] + beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
      else if(is.na(mydata[j-1]) & is.na(Tempz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc))+ beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc))) }
      else if(is.na(mydata[j-1]) & is.na(Tempz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc))+ beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1]) }
      else if(is.na(mydata[j-1]) & is.na(Tempz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_T[j],sqrt(1/tau_T_proc))+ beta4*(Schmidtz[j] - Schmidtz[j-1])}
      else if(is.na(mydata[j-1]) & !is.na(Tempz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j-1]+ beta3*Tempz[j]+ beta4*(Schmidtz[j] - Schmidtz[j-1])}
      else{lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Tempz[j] + beta4*(Schmidtz[j] - Schmidtz[j-1]) }
      
      pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
      
      #data model
      pred_obs.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j] = rnorm(nsamp,pred.Seasonal_AR_Mintemp_MinSchmidt_Diff[,j],sqrt(1/tau_obs))
    }
}
...
if(model_name=="Seasonal_AR_Minwind_MinSchmidt_Diff"){
  ...
  beta3 = out[samp,grep("beta3",colnames(out))]
  beta4 = out[samp,grep("beta4",colnames(out))]
  pred.Seasonal_AR_Minwind_MinSchmidt_Diff <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_Minwind_MinSchmidt_Diff <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  Wndz = Wnds
  ...
  for(j in 2:max(season_weeks)){
    ...
    #process model
    #filling Temp NAs
    if(is.na(mydata[j-1]) & is.na(Wndz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc)) + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & is.na(Wndz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc)) + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & !is.na(Wndz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Wndz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & !is.na(Wndz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Wndz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
    else if(!is.na(mydata[j-1]) & !is.na(Wndz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Wndz[j] + beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & is.na(Wndz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc))+ beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc))) }
    else if(!is.na(mydata[j-1]) & is.na(Wndz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc))+ beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1]) }
    else if(!is.na(mydata[j-1]) & is.na(Wndz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc))+ beta4*(Schmidtz[j] - Schmidtz[j-1])}
    else if(is.na(mydata[j-1]) & !is.na(Wndz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*Wndz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(is.na(mydata[j-1]) & !is.na(Wndz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*Wndz[j] + beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
    else if(is.na(mydata[j-1]) & !is.na(Wndz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*Wndz[j] + beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(is.na(mydata[j-1]) & is.na(Wndz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc))+ beta4*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc))) }
    else if(is.na(mydata[j-1]) & is.na(Wndz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc))+ beta4*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1]) }
    else if(is.na(mydata[j-1]) & is.na(Wndz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_min_W[j],sqrt(1/tau_W_proc))+ beta4*(Schmidtz[j] - Schmidtz[j-1])}
    else if(is.na(mydata[j-1]) & !is.na(Wndz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j-1]+ beta3*Wndz[j]+ beta4*(Schmidtz[j] - Schmidtz[j-1])}
    else{lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*Wndz[j] + beta4*(Schmidtz[j] - Schmidtz[j-1]) }
    
    pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
    
    #data model
    pred_obs.Seasonal_AR_Minwind_MinSchmidt_Diff[,j] = rnorm(nsamp,pred.Seasonal_AR_Minwind_MinSchmidt_Diff[,j],sqrt(1/tau_obs))
  }
}
...
if(model_name=="Seasonal_AR_SWradiation_MinSchmidt_Diff"){
  ...
  beta4 = out[samp,grep("beta4",colnames(out))]
  beta5 = out[samp,grep("beta5",colnames(out))]
  pred.Seasonal_AR_SWradiation_MinSchmidt_Diff <- matrix(NA,nrow=nsamp,ncol=20)
  pred_obs.Seasonal_AR_SWradiation_MinSchmidt_Diff <- matrix(NA, nrow=nsamp, ncol=20)
  year_no <- 1
  season_weeks <- c(1:20)
  lambda <- matrix(NA, nrow=nsamp, ncol=20)
  SWz = SWs
  ...
  for(j in 2:max(season_weeks)){
    ...
    #process model
    #filling Temp NAs
    if(is.na(mydata[j-1]) & is.na(SWz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc)) + beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & is.na(SWz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & !is.na(SWz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & !is.na(SWz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
    else if(!is.na(mydata[j-1]) & !is.na(SWz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(!is.na(mydata[j-1]) & is.na(SWz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + beta5*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc))) }
    else if(!is.na(mydata[j-1]) & is.na(SWz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1]) }
    else if(!is.na(mydata[j-1]) & is.na(SWz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + beta5*(Schmidtz[j] - Schmidtz[j-1])}
    else if(is.na(mydata[j-1]) & !is.na(SWz[j]) & is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(is.na(mydata[j-1]) & !is.na(SWz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1])}
    else if(is.na(mydata[j-1]) & !is.na(SWz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc)))}
    else if(is.na(mydata[j-1]) & is.na(SWz[j]) & !is.na(Schmidtz[j]) & is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+ beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 +beta5*(Schmidtz[j] - rnorm(nsamp,week_min_S[j-1],sqrt(1/tau_S_proc))) }
    else if(is.na(mydata[j-1]) & is.na(SWz[j]) & is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + beta5*(rnorm(nsamp,week_min_S[j],sqrt(1/tau_S_proc)) - Schmidtz[j-1]) }
    else if(is.na(mydata[j-1]) & is.na(SWz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))+beta4*rnorm(nsamp,week_avg[j],sqrt(1/tau_SW_proc))^2 + beta5*(Schmidtz[j] - Schmidtz[j-1])}
    else if(is.na(mydata[j-1]) & !is.na(SWz[j]) & !is.na(Schmidtz[j]) & !is.na(Schmidtz[j-1])){lambda[,j] <- beta1 + beta2*pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j-1]+ beta3*SWz[j]+ beta4*SWz[j]^2 +beta5*(Schmidtz[j] - Schmidtz[j-1])}
    else{lambda[,j] <- beta1 + beta2*mydata[j-1]+ beta3*SWz[j] +beta4*SWz[j]^2 + beta5*(Schmidtz[j] - Schmidtz[j-1]) }
    
    pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j] = rnorm(nsamp,lambda[,j],sqrt(1/tau_proc))
    
    #data model
    pred_obs.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j] = rnorm(nsamp,pred.Seasonal_AR_SWradiation_MinSchmidt_Diff[,j],sqrt(1/tau_obs))
  }
}
