model{
  ...
  for(j in 2:max(season_weeks)){
    ...
    lambda[k,j] <- beta1  + beta2*mu[k,j-1] + beta3*Temp[k,j] + beta4*Wnd[k,j]
    
    #process model for temperature
    Temp[k,j]~dnorm(week_min_T[j],tau_T_proc)
    Wnd[k,j]~dnorm(week_min_W[j],tau_W_proc)
