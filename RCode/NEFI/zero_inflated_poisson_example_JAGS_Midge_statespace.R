##Work in Progress: Turning the Zero-inflated Model into a State Space Model
...
model{
  ...
  #set an initial value of m because we start at t=2
  ...
  for(j in 2:length(x.new)){
    ...
    log(mu.new[j-1]) <- m.new[j-1]+beta.pois[1] + beta.pois[2]*x.new[j-1]

    #theta[j] is the linear combination of predictors and paramters for the bernoulli component of the model.
    logit(theta.new[j-1]) <- m.new[j-1]+beta.bern[1] + beta.bern[2]*x.new[j-1]
  }
