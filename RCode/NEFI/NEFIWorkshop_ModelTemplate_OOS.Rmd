...
#3) Convert & visualize data (no edits)

```{r}
time=as.character(dat$date)
#times<-as.Date(time)

#Separating data into testing & training
train<- subset(dat, year<2015) 
y<- round(train$totalperL*141.3707) #converting colonies per Liter to count data: volume of 2, ~1 m net tows
test<- subset(dat, year>2014)
y.oos<- round(test$totalperL*141.3707)

#Looking at response variables in testing & training data
par(mfrow=(c(2,2)))
hist(y)
hist(y.oos)
N=length(y)
...
data.RandomWalk <- list(y=y, y.oos=y.oos,N=length(y),N.oos=length(y.oos), x_ic=log(0.1),tau_ic=100,a_add=.001,r_add=.001)
variable.names.RandomWalk<- c("tau_add", "MSPE", "PD")
variable.namesout.RandomWalk<- c("tau_add", "mu", "MSPE", "PD")
...
name="RandomWalk.R" #Type in name of model script file
...
MSPEs=grep("MSPE", colnames(out))
MSPE_mean = mean(out[,MSPEs])
MSPE_mean 
MSPE_median=median(out[,MSPEs])
MSPE_median

#Log Predictive Density
PDs=grep("PD", colnames(out))
PD_logmean = log(mean(out[,PDs]))
PD_logmean
...
```{r}
times <- as.Date(as.character(train$date))
...
#10) one step ahead prediction: can plot using code above; plot out-of-sample predictions first because wide CIs; calculate means and plot predicted vs. observed

#Edit this section to match features of your model (change nsamp, sample parameters, process model)
...
#Edit this to reflect process model
