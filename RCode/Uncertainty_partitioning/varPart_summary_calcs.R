...
myfiles <- list.files(path = "./Results/Uncertainty_partitioning/w_Data_assim_1wk/", pattern = "_2015.csv")
...
colnames(datum) <- colnames(dat)

dat <- rbind(dat, datum) 

dat_z <- dat %>%
  filter(Var_type == "Initial conditions")
...
dat6 <- dat5 %>%
  ...
  mutate(Mod_name = ifelse(Mod_name == "Seasonal_AR","AR",
                           ifelse(Mod_name == "Seasonal_AR_Mintemp","AR_mintemp",
                                  ifelse(Mod_name == "Seasonal_DayLength_Quad","Quad_daylength",
                                         ifelse(Mod_name == "Seasonal_DayLength_Quad_Mintemp","Daylength_mintemp",
                                                ifelse(Mod_name == "Seasonal_RandomWalk","RW",
                                                       ifelse(Mod_name == "Seasonal_RandomWalk_Obs_error","RW_obs",Mod_name))))))) %>%
  mutate(Mod_name = factor(Mod_name, levels = c("RW","RW_obs","AR","AR_mintemp","Quad_daylength","Daylength_mintemp")))
...
dat2 <- dat %>%
  #select(-X1,-X21) %>%
  gather(X1:X40, key = "sampling_week",value = "variance")
...
dat4 <- dat3 %>%
  mutate(Mod_name = ifelse(Mod_name == "Midge_Seasonal_AR","AR",
                           ifelse(Mod_name == "Midge_Seasonal_AR_Mintemp","AR_mintemp",
                                  ifelse(Mod_name == "Midge_Seasonal_DayLength_Quad","Quad_daylength",
                                         ifelse(Mod_name == "Midge_Seasonal_DayLength_Quad_Mintemp","Daylength_mintemp",
                                                ifelse(Mod_name == "Midge_Seasonal_RandomWalk","RW",
                                                       ifelse(Mod_name == "Midge_Seasonal_RandomWalk_Obs_error","RW_obs",Mod_name))))))) %>%
  mutate(Mod_name = factor(Mod_name, levels = c("RW","RW_obs","AR","AR_mintemp","Quad_daylength","Daylength_mintemp")))
...
p1 <- ggplot(data = dat4, aes(x = Mod_name, y = exp(variance)))+
  ...
  xlab("")+
  ylab("Variance")+
  geom_text(x = 5.5, y = 90, aes(label = "1 week hindcast"), size = 6) +
  scale_y_log10(limits = c(0.003,200000))

p1

################4 week ahead hindcasts
myfiles <- list.files(path = "./Results/Uncertainty_partitioning/w_Data_assim_4wk/", pattern = "_2015.csv")
...
dat0 <- dat %>%
  gather(X1:X16, key = "season_week",value = "percent_var") 
...
dat2 <- dat %>%
  #select(-X1,-X21) %>%
  gather(X1:X36, key = "sampling_week",value = "variance")
