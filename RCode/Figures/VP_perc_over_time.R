...
plot_varMat <- function(V.pred.rel, forecast_times, year){
  ...
  if(nrow(V.pred.rel) == 3){
    ...
    par(mar=c(2, 4.1, 1, 1), mgp = c(2.5,1,0))
    plot(forecast_times[1:20], V.pred.rel[1,], ylim=c(0,1), type='n', main="", ylab="Proportion of Variance", xlab="", cex.lab = 1.5, cex.axis = 1.5, xaxs="i", yaxs="i")
    ...
    if(year == "2015"){
      abline(v = forecast_times[6], lwd = 3)
      abline(v = forecast_times[10], lwd = 3)
      abline(v = forecast_times[17], lwd = 3, lty = 2)
    } else {
...
for (i in 1:length(model_names)){
  ...
  varMat_2015 <- read_csv(file = file.path("./Results/Uncertainty_partitioning/w_Data_assim_1wk/",paste0("Midge_",model_names[i],"_varMat_2015.csv")))
  ...
  tiff(file = file.path("C:/Users/Mary Lofton/Dropbox/Ch5/Final_figs/",paste0(model_names[i],"_V.pred.rel.2015.tif")),
       width = 8, height = 6, units = "in", res = 300)
  plot_varMat(V.pred.rel = varMat_2015,
  ...
  plot_varMat(V.pred.rel = varMat_2016,
              forecast_times = forecast_times[21:40],
              year = "2016")
  dev.off()
...
plot_varMat <- function(V.pred.rel, forecast_times, year){
  ...
  if(nrow(V.pred.rel) == 3){
    ...
    layout(rbind(1,2), heights=c(7,1))
    par(mar=c(2, 4.1, 1, 1), mgp = c(2.5,1,0))
    plot(forecast_times[4:19], V.pred.rel[1,], ylim=c(0,1), type='n', main="", ylab="Proportion of Variance", xlab="", cex.lab = 1.5, cex.axis = 1.5, xaxs="i", yaxs="i")
...
for (i in 1:length(model_names)){
  ...
  varMat_2015 <- read_csv(file = file.path("./Results/Uncertainty_partitioning/w_Data_assim_4wk/",paste0("Midge_",model_names[i],"_varMat_2015.csv")))
  ...
  tiff(file = file.path("C:/Users/Mary Lofton/Dropbox/Ch5/Final_figs/",paste0(model_names[i],"_V.pred.rel.2015_4wk.tif")),
       width = 8, height = 6, units = "in", res = 300)
  plot_varMat(V.pred.rel = varMat_2015,
              forecast_times = forecast_times[1:20],
              year = "2015")
