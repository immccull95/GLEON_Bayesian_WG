...
sw <- read_csv("./Datasets/Sunapee/RawData/NLDAS_SunapeeMet_1979_2016.csv") %>%
  mutate(date = date(datetime)) %>%
  group_by(date) %>%
  summarize(SW_median_daily = median(ShortWaveRad_Wperm2, na.rm = TRUE))

sw1 <- left_join(cleaned_dat2, sw, by = "date")

plot(sw1$SW_median_daily,log(sw1$totalperL))

ggplot(data = sw1, aes(x = season_week, y = SW_median_daily))+
  geom_point(size = 1)+
  geom_line(size = 1)+
...
sw_seasonal <- sw1 %>%
  select(year, season_week, SW_median_daily) %>%
  spread(key = season_week, value = SW_median_daily) %>%
  select(-year)
...
sw2 <- sw1 %>%
  select(date, year, season_week, SW_median_daily)

write.csv(sw2, "./Datasets/Sunapee/SummarizedData/Midge_year_by_week_SW_forecast_25FEB20.csv", row.names = FALSE)
