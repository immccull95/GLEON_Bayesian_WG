...
for(i in 1:nchain){
  ...
  init[[i]] <- list(tau_add=1/var(diff(y.samp)),
                    theta = sum(y.samp == 0)/length(y.samp))
}
```
...
```{r}
j.model   <- jags.model (file = textConnection(RandomWalkZIP),
                             data = data,
                             inits = init,
                             n.chains = 3)
```
...
```{r}
jags.out   <- coda.samples (model = j.model,
                            variable.names = c("tau_add", "mu","theta"),
                                n.iter = 5000)

save(jags.out, file = "C:/Users/Mary Lofton/Documents/GLEON/Bayesian/RandomWalkZIP_Newbury.RData")
...
#random draw from bernoulli for zero-inflation
b <- rbernoulli(nsamp,theta)

#blend distributions and exponentiate to get appropriate input for poisson
m <- exp(pred[,t]) * b + 1E-10

#predictive interval for y (including data model)
